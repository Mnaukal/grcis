...
if (context.ContainsKey("animator")) {
    ...
    string keyframes_file = Path.Combine(Path.GetDirectoryName((string)context[PropertyName.CTX_SCRIPT_PATH]), "DoFExample.yaml");
...
if (context.ContainsKey(PropertyName.CTX_SUPERSAMPLING) && (int)context[PropertyName.CTX_SUPERSAMPLING] == 1)
